...
func main() {
	...
	// 注册任务执行函数，然后启动任务引擎（必须在所有服务 wiring 完成后调用）。
	// Pass serverRootCtx so engine-dispatched task goroutines can be cancelled on graceful shutdown.
	if services.TaskService != nil {
		registerTaskResumeHandlers(services, repos)
		// 迁移期安全阀：除已完成"仅创建任务，执行权交给引擎"改造的类型之外，其余任务类型仍由
		// 各 handler/service 内部直接执行（裸 goroutine 或 RunTracked），暂时排除在引擎调度之外，
		// 避免引擎和调用方同时争抢执行同一条任务。全项目所有已注册 taskType 现已全部完成迁移
		// （HTTP handler 站点 + chapter_service/asset_service/novel_analysis_service/
		// rewrite_service/video_synthesis_service 里内部链式创建的任务），此列表现为全量列表；
		// "platform_publish" 未注册 resume handler（改用 RunTracked，不接受引擎调度，见
		// platform_publish_service.go），因此不需要出现在这里。
		services.TaskService.ExcludeAllRegisteredExcept(
			service.TaskTypeThreeView,
			service.TaskTypeCharGen,
			...
			service.TaskTypeAssetGen,
			service.TaskTypeSFXGen,
			service.TaskTypeVoiceGen,
			service.TaskTypeLipSync,
			"bgm_analyze",
			"bgm_generate",
			service.TaskTypeChapterGen,
			service.TaskTypeChapterSummaryBatch,
			service.TaskTypeChapterReviewBatch,
			service.TaskTypeChapterReview,
			service.TaskTypeChapterOutlineReview,
			"outline_review_batch",
			service.TaskTypeChapterRewriteInstr,
			service.TaskTypeChapterPostProcess,
			service.TaskTypeBatchChapterGen,
			...
			service.TaskTypeSkillGen,
			service.TaskTypePlotExtract,
			service.TaskTypeImport,
			service.TaskTypeNovelAnalysis,
			service.TaskTypeRewriteAnalysis,
			service.TaskTypeRewriteChapters,
			service.TaskTypeCrawlJob,
			service.TaskTypeVideoSynthesis,
		)
		services.TaskService.Boot(serverRootCtx)
		services.TaskService.StartEngine(serverRootCtx)
	}
