...
# =============================================================================
Write-Step "1 / 7  检查 winget（Windows 包管理器）"
# =============================================================================
$HasWinget = $false
try {
...
if (-not $GoInstalled) {
    if ($HasWinget) {
        ...
        Invoke-WebRequest -Uri $GoUrl -OutFile $GoTmp -UseBasicParsing
        Write-Info "安装 Go..."
        Start-Process msiexec.exe -Wait -ArgumentList "/i `"$GoTmp`" /quiet /norestart"
        Remove-Item $GoTmp -Force
    }
    Refresh-Path
}
...
if (-not $NodeInstalled) {
    if ($HasWinget) {
        ...
        Invoke-WebRequest -Uri "$NodeUrl$NodeMsi" -OutFile $NodeTmp -UseBasicParsing
...
# 启动 MySQL 服务
foreach ($svc in @("MySQL80","MySQL","MySQL84")) {
    if (Get-Service -Name $svc -ErrorAction SilentlyContinue) {
...
if (-not $RedisExe) {
    if ($HasWinget) {
        # Windows 版 Redis（Memurai 兼容 Redis API，或 Redis Stack）
        winget install --id Redis.Redis --silent --accept-package-agreements --accept-source-agreements 2>&1 | Out-Null
        Refresh-Path
        try { $RedisExe = (Get-Command redis-server -ErrorAction Stop).Source } catch { }
    }
    if (-not $RedisExe) {
        Write-Warn "未找到 Redis，将下载便携版..."
        $RedisDir = "C:\Redis"
        New-Item -ItemType Directory -Path $RedisDir -Force | Out-Null
        $RedisZip = "$env:TEMP\redis.zip"
        Invoke-WebRequest -Uri "https://github.com/microsoftarchive/redis/releases/download/win-3.2.100/Redis-x64-3.2.100.zip" `
        ...
        if ($UserPath -notlike "*$RedisDir*") {
            [System.Environment]::SetEnvironmentVariable("Path", "$UserPath;$RedisDir", "User")
        }
    }
}
...
# 启动 Redis（后台）
try { $null = redis-cli ping 2>&1 } catch { }
$RedisPing = try { (redis-cli ping 2>&1) -eq "PONG" } catch { $false }
if (-not $RedisPing) {
...
$DbPass  = Read-Host   "  密码          (空=无密码)" -AsSecureString
...
$cfg = $cfg -replace 'host: "localhost"\s+# \[必填\]', "host: `"$DbHost`""
$cfg = $cfg -replace 'port: 3306',                     "port: $DbPort"
$cfg = $cfg -replace 'database: "inkframe"\s+# \[必填\]',"database: `"$DbName`""
$cfg = $cfg -replace 'username: "root"\s+# \[必填\]',  "username: `"$DbUser`""
$cfg = $cfg -replace 'password: ""\s+# \[必填\]',      "password: `"$DbPassPlain`""
...
if ($Mode -eq "production") {
    ...
    $cfg = $cfg -replace 'jwt_secret: "inkframe-dev-secret-change-in-production-2024"', `
                          "jwt_secret: `"$jwt`""
}
...
if ($DbPassPlain) { $MysqlArgs += " -p`"$DbPassPlain`"" }
$CreateDb  = "CREATE DATABASE IF NOT EXISTS ``$DbName`` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
try {
    $result = & $MysqlExe -h $DbHost -P $DbPort -u $DbUser $(if ($DbPassPlain) { "-p$DbPassPlain" }) `
        -e $CreateDb 2>&1
...
foreach ($name in @("inkframe-backend","inkframe-frontend")) {
    ...
    if (Test-Path $pf) {
        $pid = [int](Get-Content $pf -ErrorAction SilentlyContinue)
        Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue
...
for ($i = 0; $i -lt 20; $i++) {
    try {
        $r = Invoke-WebRequest -Uri "http://localhost:$BePort/health" -UseBasicParsing -TimeoutSec 1 -ErrorAction SilentlyContinue
