...
# ti2v-5B 首次下载约 10GB，设 1800s；A14B 约 30GB，设 3600s
...
def handler(event, context):
    ...
    根据 Content-Type 自动路由：
      application/json      → POST /generate      (T2V)
      multipart/form-data   → POST /generate/i2v  (I2V)
    """
    ...
    logger.info(f"Forwarding to {path}  content-type={content_type[:60]}")
