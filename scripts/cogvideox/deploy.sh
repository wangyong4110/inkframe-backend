...
')
cd "${SCRIPT_DIR}"
FC_ENV="{\"ALIBABA_ACCESS_KEY\":\"${ALIBABA_ACCESS_KEY}\",\"ALIBABA_ACCESS_SECRET\":\"${ALIBABA_ACCESS_SECRET}\",\"REGION_ID\":\"${REGION_ID}\",\"ECS_INSTANCE_ID\":\"${ECS_INSTANCE_ID}\",\"ECS_PRIVATE_IP\":\"${ECS_PRIVATE_IP}\",\"TIMEOUT_START\":\"${TIMEOUT_START:-1800}\",\"TIMEOUT_INFER\":\"${TIMEOUT_INFER:-600}\",\"AUTO_STOP\":\"${AUTO_STOP:-true}\"}"
aliyun fc POST /services --header "Content-Type=application/json" \
    --body "{\"serviceName\":\"${FC_SERVICE_NAME}\",\"description\":\"T2V\",\"vpcConfig\":{\"vpcId\":\"${VPC_ID}\",\"vSwitchIds\":[\"${VSWITCH_ID}\"],\"securityGroupId\":\"${SECURITY_GROUP_ID}\"}}" \
    2>/dev/null && ok "FC 服务" || warn "已存在"
...
aliyun fc POST "/services/${FC_SERVICE_NAME}/functions" --header "Content-Type=application/json" \
    --body "${FUNC}" 2>/dev/null && ok "FC 函数" \
    || { warn "更新中..."; aliyun fc PUT "/services/${FC_SERVICE_NAME}/functions/${FC_FUNCTION_NAME}" \
         --header "Content-Type=application/json" --body "${FUNC}" > /dev/null && ok "FC 更新"; }
...
echo ""
echo "  # 文本转视频"
echo "  curl -X POST http://${ECS_PRIVATE_IP}:8000/generate -H 'Content-Type: application/json' -d '{\"prompt\":\"a cat walking on the beach\"}'"
echo "  # 图片转视频"
echo "  curl -X POST http://${ECS_PRIVATE_IP}:8000/generate/i2v -F 'image=@input.jpg' -F 'prompt=the cat starts running'"
