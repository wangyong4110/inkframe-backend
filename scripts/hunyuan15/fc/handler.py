...
def wait_for_service():
    ...
    while time.time() < deadline:
        ...
        if time.time() - last_log >= 30:
            logger.info(f"Waiting {int(time.time()-(deadline-TIMEOUT_START))}s")
            last_log = time.time()
        time.sleep(8)
    return False
...
def handler(event, context):
    ...
    path = "/generate/i2v" if "multipart" in ct else "/generate"
    try:
        start_instance()
        if not wait_for_service():
            return json.dumps({"error": "startup timeout"})
        resp = requests.post(f"{INFER_BASE}{path}", data=body_bytes,
                             headers={"Content-Type": ct}, timeout=TIMEOUT_INFER)
