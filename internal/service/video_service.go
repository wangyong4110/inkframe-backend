...
▸ image_prompt（图片生成专用提示词，English only，必填）
  - 专为 Stable Diffusion / Flux / DALL-E 等图片生成模型优化，与 description 不同，需加入画质词和风格词
  - 结构（按顺序拼接）：
...
▸ video_prompt（视频生成专用提示词，English only，必填）
...
▸ negative_prompt（负向提示词，English only，必填）
  - 针对本镜头内容的具体缺陷排除词，而非通用固定词表
...
func (s *VideoService) parseStoryboardResult(videoID uint, chapterID *uint, result string) ([]*model.StoryboardShot, error) {
	...
	var rawShots []struct {
		...
		Description string  `json:"description"`   // 英文画面描述（客观）
		ImagePrompt string  `json:"image_prompt"`  // 图片生成专用提示词（含画质词）
		VideoPrompt   string  `json:"video_prompt"`   // 视频生成专用提示词（含运动描述）
		NegativePrompt string  `json:"negative_prompt"` // 负向提示词（场景定制）
