...
// batchChapterPlan 批量生成时单章的创作蓝图（从大纲和 DB 章节提取）
type batchChapterPlan struct {
	ChapterNo     int
	Title         string
	ActNo         int
	TensionLevel  int
	...
	HookType      string
	Summary       string
	SceneOutline  string // JSON 字符串
	TextOutline   string // 文字大纲
}

// batchChapterOutput AI 返回的单章结果
type batchChapterOutput struct {
	ChapterNo   int    `json:"chapter_no"`
	Title       string `json:"title"`
...
type batchChapterResponse struct {
	Chapters []batchChapterOutput `json:"chapters"`
}

// BatchGenerateChaptersSingleCall 在一次 LLM 调用中完成所有指定章节的正文生成。
// 先构建包含世界观·角色·伏笔的叙事圣经 + 所有章节大纲的综合 prompt，
// 单次调用后解析 JSON 并 upsert 所有章节；摘要与角色快照在后台异步提取。
func (s *ChapterService) BatchGenerateChaptersSingleCall(
	tenantID, novelID uint,
	...
	// ── 构建叙事圣经（世界观 + 角色快照 + 近期章节摘要）──────────────────────────
	narrativeBible := s.buildGlobalContext(novelID, 1, novel)
	foreshadowHints := s.buildForeshadowHints(novelID, 1)
	characterRegistry := s.buildCharacterRegistry(novelID)
	...
	// ── 构建各章创作蓝图 ────────────────────────────────────────────────────────
	plans := make([]batchChapterPlan, 0, len(chapters))
	for _, ch := range chapters {
		meta := s.extractChapterMeta(novelID, ch.ChapterNo)
		...
		sceneOutline := ch.SceneOutline
		if strings.TrimSpace(sceneOutline) == "{}" {
			sceneOutline = ""
		}

		plans = append(plans, batchChapterPlan{
			...
			TextOutline:   ch.Outline,
		})
	}

	// ── 渲染单次调用的综合 prompt ──────────────────────────────────────────────
	prompt, err := renderPrompt("batch_chapter_content", map[string]interface{}{
		"NovelTitle":        novel.Title,
		"Genre":             novel.Genre,
	...
	if err != nil {
		return nil, fmt.Errorf("render batch prompt: %w", err)
	}

	// ── 单次 LLM 调用 ─────────────────────────────────────────────────────────
	// 估算 max tokens：每章约 wordCount 字 ≈ wordCount*2 tokens（中文），加 50% 缓冲
	maxTokens := wordCount * len(chapters) * 3
	if maxTokens < 8192 {
		maxTokens = 8192
	}
	if maxTokens > 131072 {
	...
	overrides := StoryboardOverrides{MaxTokens: maxTokens}
	_ = modelOverride // model override applied via novel.AIModel; explicit override not yet supported here

	logger.Printf("[BatchGenSingle] novelID=%d chapters=%d maxTokens=%d", novelID, len(chapters), maxTokens)

	resp, genErr := s.aiService.GenerateWithProvider(tenantID, novelID, "batch_chapter_gen", prompt, "", overrides)
	if genErr != nil {
		return nil, fmt.Errorf("single-call generation failed: %w", genErr)
	}

	// ── 解析 JSON 响应 ────────────────────────────────────────────────────────
	cleaned := extractJSONObject(strings.TrimSpace(resp))
	var batchResp batchChapterResponse
	if parseErr := json.Unmarshal([]byte(cleaned), &batchResp); parseErr != nil {
		return nil, fmt.Errorf("parse batch response: %w (preview: %s)", parseErr, truncateForPrompt(cleaned, 300))
	}
	if len(batchResp.Chapters) == 0 {
		return nil, fmt.Errorf("batch response contained no chapters")
	}

	// 建立 chapter_no → output 映射，容错顺序错乱
	outputMap := make(map[int]batchChapterOutput, len(batchResp.Chapters))
	for _, out := range batchResp.Chapters {
		outputMap[out.ChapterNo] = out
	}

	// ── upsert 所有章节到 DB ──────────────────────────────────────────────────
	var saved []*model.Chapter
	for _, plan := range plans {
		out, ok := outputMap[plan.ChapterNo]
		if !ok {
			logger.Printf("[BatchGenSingle] chapter %d missing from AI response", plan.ChapterNo)
			continue
		}
		if len([]rune(out.Content)) < 100 {
			logger.Printf("[BatchGenSingle] chapter %d content too short (%d chars), skipping", plan.ChapterNo, len([]rune(out.Content)))
			continue
		}
		...
		var chapter *model.Chapter
		if existing, _ := s.chapterRepo.GetByNovelAndChapterNo(novelID, plan.ChapterNo); existing != nil {
			existing.Title = title
			existing.Content = out.Content
			existing.WordCount = countChineseChars(out.Content)
			existing.TensionLevel = plan.TensionLevel
			existing.ActNo = plan.ActNo
			existing.EmotionalTone = plan.EmotionalTone
			existing.HookType = plan.HookType
			existing.ChapterHook = out.ChapterHook
			existing.Status = "completed"
			if updateErr := s.chapterRepo.Update(existing); updateErr != nil {
				logger.Printf("[BatchGenSingle] update chapter %d failed: %v", plan.ChapterNo, updateErr)
				continue
			}
			chapter = existing
		} else {
			chapter = &model.Chapter{
				...
				Title:         title,
				Content:       out.Content,
				WordCount:     countChineseChars(out.Content),
				TensionLevel:  plan.TensionLevel,
				ActNo:         plan.ActNo,
				EmotionalTone: plan.EmotionalTone,
				HookType:      plan.HookType,
				ChapterHook:   out.ChapterHook,
				Status:        "completed",
			}
			if createErr := s.chapterRepo.Create(chapter); createErr != nil {
				logger.Printf("[BatchGenSingle] create chapter %d failed: %v", plan.ChapterNo, createErr)
				continue
			}
		}
	...
	// ── 异步后处理：摘要提取 + 角色状态快照（非阻塞，完成后供后续章节生成使用）────
	go func(chs []*model.Chapter) {
		novelSvcForSnapshot := &NovelService{
			novelRepo:     s.novelRepo,
			chapterRepo:   s.chapterRepo,
			aiService:     s.aiService,
			characterRepo: s.characterRepo,
			snapshotRepo:  s.snapshotRepo,
		}
		for _, ch := range chs {
			if s.narrativeSvc != nil && ch.Summary == "" {
				if sum, sumErr := s.narrativeSvc.GenerateChapterSummary(tenantID, ch, novel.Title); sumErr == nil {
					ch.Summary = sum
					if updateErr := s.chapterRepo.Update(ch); updateErr != nil {
						logger.Printf("[BatchGenSingle] update summary ch%d: %v", ch.ChapterNo, updateErr)
					}
				} else {
					logger.Printf("[BatchGenSingle] summary ch%d: %v", ch.ChapterNo, sumErr)
				}
			}
			novelSvcForSnapshot.writeCharacterSnapshots(tenantID, ch)
		}
	}(saved)

	logger.Printf("[BatchGenSingle] done: novelID=%d requested=%d saved=%d", novelID, len(chapters), len(saved))
	return saved, nil
}
