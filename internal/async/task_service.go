...
// TaskNotifier is the interface for sending failure notifications.
// Implemented by *service.NotificationService — defined here to avoid async importing service.
type TaskNotifier interface {
	Send(tenantID, userID uint, eventType, title, body, entityType string, entityID uint, linkPath string) error
}
...
type TaskService struct {
	...
	notifSvc         TaskNotifier                       // optional: sends in-app notifications on task failure
...
func (s *TaskService) WithNotificationService(svc TaskNotifier) *TaskService {
	s.notifSvc = svc
	return s
}
