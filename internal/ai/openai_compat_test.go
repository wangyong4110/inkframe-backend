...
func TestOpenAICompatProvider_Generate_UsesRequestModel(t *testing.T) {
	...
	p := NewOpenAICompatProvider("p", "key", server.URL, "default-model", nil, 0)
	_, err := p.Generate(context.Background(), &GenerateRequest{Model: "override-model", Messages: []ChatMessage{{Role: "user", Content: "hi"}}})
	if err != nil {
		t.Fatalf("Generate() error: %v", err)
	}
...
func TestOpenAICompatProvider_ImageGenerate_NotSupported(t *testing.T) {
	...
	if _, err := p.ImageGenerate(context.Background(), &ImageGenerateRequest{}); err == nil {
		t.Error("expected error, got nil")
	}
}
...
var _ AIProvider = (*OpenAICompatProvider)(nil)
