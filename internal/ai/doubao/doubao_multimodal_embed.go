...
func (p *DoubaoProvider) EmbedMultimodal(ctx context.Context, req *ai.MultimodalEmbedRequest) (*ai.MultimodalEmbedResponse, error) {
	start := time.Now()
	...
	out := &ai.MultimodalEmbedResponse{
	...
	if resp.Data.SparseEmbedding != nil {
		points := make([]ai.SparseEmbedPoint, 0, len(*resp.Data.SparseEmbedding))
		for _, p := range *resp.Data.SparseEmbedding {
			points = append(points, ai.SparseEmbedPoint{Index: p.Index, Value: p.Value})
		}
