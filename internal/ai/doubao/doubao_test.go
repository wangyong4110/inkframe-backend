...
func TestDoubaoProvider_ImageGenerate(t *testing.T) {
	...
	p := NewDoubaoProvider("key", server.URL, "", 0)
	resp, err := p.ImageGenerate(context.Background(), &ai.ImageGenerateRequest{Model: "doubao-seedream-4-0-250828", Prompt: "a cat", Size: "1024x1024"})
	if err != nil {
...
func TestDoubaoProvider_ImageGenerate_SensitiveContentError(t *testing.T) {
	...
	p := NewDoubaoProvider("key", server.URL, "", 0)
	resp, err := p.ImageGenerate(context.Background(), &ai.ImageGenerateRequest{Prompt: "x"})
	if err != nil {
	...
	if !strings.HasPrefix(resp.Error, "Seedream 错误: "+ErrPrefixSensitiveContent) {
		t.Errorf("Error = %q, want ai.ErrPrefixSensitiveContent marker", resp.Error)
	}
}
...
func TestDoubaoProvider_ImageGenerate_PartialFailureInGroup(t *testing.T) {
	...
	p := NewDoubaoProvider("key", server.URL, "", 0)
	resp, err := p.ImageGenerate(context.Background(), &ai.ImageGenerateRequest{Prompt: "x", SequentialImageGeneration: "auto"})
	if err != nil {
		t.Fatalf("ImageGenerate() error: %v", err)
	}
...
func TestDoubaoProvider_AudioGenerate(t *testing.T) {
	...
	resp, err := p.AudioGenerate(context.Background(), &ai.AudioGenerateRequest{Text: "hello", Voice: "zh_female_1"})
	if err != nil {
...
func TestDoubaoProvider_AudioGenerate_ErrorStatus(t *testing.T) {
	...
	p := NewDoubaoProvider("key", server.URL, "", 0)
	if _, err := p.AudioGenerate(context.Background(), &ai.AudioGenerateRequest{Text: "hi"}); err == nil {
		t.Error("expected error")
	}
}
