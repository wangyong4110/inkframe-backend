...
func (p *TencentTTSProvider) AudioGenerate(ctx context.Context, req *ai.AudioGenerateRequest) (*ai.AudioResponse, error) {
	start := time.Now()
	...
	// 转换时间戳
	var subtitles []ai.TTSSubtitle
	for _, s := range result.Response.Subtitles {
		subtitles = append(subtitles, ai.TTSSubtitle{
