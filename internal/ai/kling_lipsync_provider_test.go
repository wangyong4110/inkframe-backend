...
func TestKlingLipSyncProvider_GenerateLipSync_ValidatesRequiredFields(t *testing.T) {
	...
	if _, err := p.GenerateLipSync(ctx, &LipSyncRequest{AudioURL: "https://x/a.mp3"}); err == nil {
		t.Error("expected error when ImageURL is empty")
	}
	if _, err := p.GenerateLipSync(ctx, &LipSyncRequest{ImageURL: "https://x/a.png"}); err == nil {
...
func TestKlingLipSyncProvider_GenerateLipSync_RealCall(t *testing.T) {
	...
	task, err := p.GenerateLipSync(ctx, &LipSyncRequest{
		ImageURL: imageURL,
		AudioURL: audioURL,
	})
	if err != nil {
