...
func init() {
	ai.RegisterImageEngineTraits(ProviderNameKlingImage, ai.ImageEngineTraits{
...
func (p *KlingImageProvider) ImageGenerate(ctx context.Context, req *ai.ImageGenerateRequest) (*ai.ImageResponse, error) {
	start := time.Now()
	...
	if img, ok := body["image"].(string); ok && img != "" {
		imageType = ai.imgRefTypeLabel(img)
	}
