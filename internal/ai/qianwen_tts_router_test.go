...
func TestQianwenTTSRouter_UnsupportedMethods(t *testing.T) {
	...
	if _, err := r.Generate(ctx, &GenerateRequest{}); err == nil {
	...
	if _, err := r.GenerateStream(ctx, &GenerateRequest{}); err == nil {
		t.Error("GenerateStream should error")
	}
	if _, err := r.Embed(ctx, "x"); err == nil {
		t.Error("Embed should error")
	}
	if _, err := r.ImageGenerate(ctx, &ImageGenerateRequest{}); err == nil {
...
func TestQianwenTTSRouter_AudioGenerate_Routing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := r.AudioGenerate(context.Background(), &AudioGenerateRequest{
				Text:  "hello",
				Voice: tt.voice,
			})
			if err == nil {
...
func TestQianwenTTSRouter_AudioGenerate_RealCall(t *testing.T) {
	...
	resp, err := r.AudioGenerate(context.Background(), &AudioGenerateRequest{
		Text:  "你好，世界",
		Voice: "longxiaochun",
	})
	if err != nil {
