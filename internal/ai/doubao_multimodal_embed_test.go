...
func TestDoubaoProvider_EmbedMultimodal_UnsupportedInputType(t *testing.T) {
	...
	_, err := p.EmbedMultimodal(context.Background(), &MultimodalEmbedRequest{
		Input: []MultimodalEmbedItem{{Type: "audio_url"}},
	})
	if err == nil {
...
func TestDoubaoProvider_ImplementsMultimodalEmbedder(t *testing.T) {
	var p AIProvider = NewDoubaoProvider("key", "", "", 0)
	if _, ok := p.(MultimodalEmbedder); !ok {
		t.Fatal("DoubaoProvider does not implement MultimodalEmbedder")
	}
}
...
func TestDoubaoProvider_EmbedMultimodal_RealCall(t *testing.T) {
	...
	resp, err := p.EmbedMultimodal(context.Background(), &MultimodalEmbedRequest{
		Model: model,
		Input: []MultimodalEmbedItem{{Type: "text", Text: "hello world"}},
	})
	if err != nil {
...
var _ MultimodalEmbedder = (*DoubaoProvider)(nil)
