...
func TestOpenAIProvider_Generate_CompletionsEndpointForDavinci(t *testing.T) {
	...
	p := NewOpenAIProvider("key", server.URL, "", 0)
	_, err := p.Generate(context.Background(), &GenerateRequest{Model: "text-davinci-003", Messages: []ChatMessage{{Role: "user", Content: "hi"}}})
	if err != nil {
		t.Fatalf("Generate() error: %v", err)
	}
...
func TestOpenAIProvider_Generate_VisionMessageUsesArrayContent(t *testing.T) {
	...
	req := &GenerateRequest{
		Messages: []ChatMessage{{Role: "user", Content: "describe", ImageURLs: []string{"https://example.com/img.png"}}},
	}
...
func TestOpenAIProvider_ImageGenerate_DallE(t *testing.T) {
	...
	p := NewOpenAIProvider("key", server.URL, "", 0)
	resp, err := p.ImageGenerate(context.Background(), &ImageGenerateRequest{Model: "dall-e-3", Prompt: "a cat", Size: "1024x1024"})
	if err != nil {
...
func TestOpenAIProvider_ImageGenerate_DallEError(t *testing.T) {
	...
	p := NewOpenAIProvider("key", server.URL, "", 0)
	resp, err := p.ImageGenerate(context.Background(), &ImageGenerateRequest{Model: "dall-e-3", Prompt: "x"})
	if err != nil {
...
func TestOpenAIProvider_ImageGenerate_NonDallEModel(t *testing.T) {
	...
	resp, err := p.ImageGenerate(context.Background(), &ImageGenerateRequest{Model: "stable-diffusion-xl", Prompt: "x"})
	if err != nil {
		t.Fatalf("ImageGenerate() error: %v", err)
	}
	if resp.Error == "" {
...
func TestOpenAIProvider_AudioGenerate(t *testing.T) {
	...
	p := NewOpenAIProvider("key", server.URL, "", 0)
	resp, err := p.AudioGenerate(context.Background(), &AudioGenerateRequest{Text: "hello", Voice: "alloy", Speed: 1.0})
	if err != nil {
...
func TestOpenAIProvider_AudioGenerate_ErrorStatus(t *testing.T) {
	...
	p := NewOpenAIProvider("key", server.URL, "", 0)
	_, err := p.AudioGenerate(context.Background(), &AudioGenerateRequest{Text: "hi", Voice: "alloy"})
	if err == nil {
		t.Fatal("expected error")
	}
}
