...
func (p *VolcengineProvider) ImageGenerate(ctx context.Context, req *ai.ImageGenerateRequest) (*ai.ImageResponse, error) {
	...
	if err != nil {
		return &ai.ImageResponse{Error: err.Error(), LatencyMs: time.Since(start).Milliseconds()}, nil
	}
...
func (p *VolcengineProvider) buildSubmitParams(reqKey string, req *ai.ImageGenerateRequest) map[string]interface{} {
	...
	case VolcModelJimengSeedream46:
		...
		// CFGScale → scale int [1,100]
		if req.CFGScale > 0 {
			normalized := (req.CFGScale - 1) / 9
			if normalized < 0 {
				normalized = 0
			}
			if normalized > 1 {
			...
			params["scale"] = int(normalized*99) + 1
		}
	}
...
func init() {
	...
	ai.RegisterImageEngineTraits(ProviderNameVolcengineVisual, ai.ImageEngineTraits{
