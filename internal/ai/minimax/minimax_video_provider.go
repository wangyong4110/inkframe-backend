...
func init() {
	ai.RegisterVideoEngineTraits(ProviderNameMinimaxVideo, ai.VideoEngineTraits{
...
func (p *MinimaxVideoProvider) GenerateVideo(ctx context.Context, req *ai.VideoGenerateRequest) (*ai.VideoTask, error) {
	isI2V := req.ImageURL != ""
	...
	if status != http.StatusOK {
		return nil, fmt.Errorf("minimax-video: submit failed: HTTP %d: %s", status, ai.Truncate(string(respBody), 300))
	}
...
func (p *MinimaxVideoProvider) queryTask(ctx context.Context, taskID string) (*minimaxQueryResult, error) {
	...
	if status != http.StatusOK {
		return nil, fmt.Errorf("minimax-video: query task failed: HTTP %d: %s", status, ai.Truncate(string(respBody), 300))
	}
...
func (p *MinimaxVideoProvider) GetVideoStatus(ctx context.Context, taskID string) (*ai.VideoTaskStatus, error) {
	result, err := p.queryTask(ctx, taskID)
	if err != nil {
		return nil, err
	}

	ts := &ai.VideoTaskStatus{
		TaskID: result.TaskID,
...
func (p *MinimaxVideoProvider) GetVideoURL(ctx context.Context, taskID string) (string, error) {
	...
	if status != http.StatusOK {
		return "", fmt.Errorf("minimax-video: retrieve file failed: HTTP %d: %s", status, ai.Truncate(string(respBody), 300))
	}
...
var _ ai.VideoProvider = (*MinimaxVideoProvider)(nil)
